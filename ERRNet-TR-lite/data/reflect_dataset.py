...
def my_make_dataset(dir, fns=None):
    m_images,b_images,r_images = [],[],[]
    assert os.path.isdir(dir), '%s is not a valid directory' % dir

    dirs=os.listdir(dir)
    for subdir in dirs:
        single_dir=join(dir,subdir)
        for j in ["_m","_b","_r"]:
            filename=subdir+j+".jpg"
            if j=="_m":
                m_images.append(join(single_dir,filename))
            elif j=="_b":
                b_images.append(join(single_dir,filename))
            elif j=="_r":
                r_images.append(join(single_dir,filename))

    return m_images,b_images,r_images
...
def my_paired_transforms(img_1, img_2, img_3, unaligned_transforms=False):
...
class myCEILDataset(BaseDataset):
    def __init__(self, datadir, fns=None, size=None, enable_transforms=True, low_sigma=2, high_sigma=5, low_gamma=1.3, high_gamma=1.3):
        super(myCEILDataset, self).__init__()
        self.size = size
        self.datadir = datadir
        self.enable_transforms = enable_transforms

        self.M_paths,self.B_paths,self.R_paths = my_make_dataset(datadir, fns)
        if size is not None:
            self.paths = self.paths[:size]
    
    def data_synthesis(self, t_img, r_img, i_img):
        if self.enable_transforms:
            t_img, r_img, i_img = my_paired_transforms(t_img, r_img, i_img)
    ...
    def __getitem__(self, index):
        index_B = index % len(self.B_paths)
        # index_R = index % len(self.R_paths)
        # index_M = index % len(self.M_paths)
        
        B_path = self.B_paths[index_B]
        R_path = self.R_paths[index_B]
        M_path = self.M_paths[index_B]
        
        t_img = Image.open(B_path).convert('RGB')
        r_img = Image.open(R_path).convert('RGB')
        i_img = Image.open(M_path).convert('RGB')

        B, R, M = self.data_synthesis(t_img, r_img, i_img)
...
class CEILTestDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        t_img = Image.open(join(self.datadir, 'transmission_layer', fn.replace("input", "label1"))).convert('RGB')
