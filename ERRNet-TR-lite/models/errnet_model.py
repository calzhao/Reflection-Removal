            ...
            if savedir is not None:
                if self.data_name is not None:
                    ...
                    if suffix is not None:
                        ...
                        Image.fromarray(output_r.astype(np.uint8)).save(join(savedir, name, '{}_r.png'.format(self.opt.name, suffix)))
                    else:
                        Image.fromarray(output_i.astype(np.uint8)).save(join(savedir, name, '{}.png'.format(self.opt.name)))
                        Image.fromarray(output_r.astype(np.uint8)).save(join(savedir, name, '{}_r.png'.format(self.opt.name)))
...
class ERRNetModel(ERRNetBase):
    ...
    def print_network(self):
        ...
        networks.print_network(self.net_r)
    ...
    def initialize(self, opt):
        ...
        net_i, net_r = arch.__dict__[self.opt.inet](in_channels, 3)
        self.net_i,self.net_r = net_i.to(self.device),net_r.to(self.device)
        networks.init_weights(self.net_i, init_type=opt.init_type) # using default initialization as EDSR
        networks.init_weights(self.net_r, init_type=opt.init_type) # using default initialization as EDSR
        ...
        if self.isTrain:
            ...
            self.optimizer_R = torch.optim.Adam(self.net_r.parameters(), 
                lr=opt.lr, betas=(0.9, 0.999), weight_decay=opt.wd)
            self._init_optimizer([self.optimizer_R])
    ...
    def backward_G(self):
        ...
        for p in self.net_r.parameters():
            p.requires_grad = False
        for p in self.net_i.parameters():
            p.requires_grad = True
        ...
        self.loss_icnn_pixel_r = None
        self.loss_icnn_remake = None
        ...
        if self.aligned:
            ...
            self.loss_icnn_remake = self.loss_dic['I_pixel'].get_loss(
                self.output_r+self.output_i, self.target_I) 
            
            self.loss_icnn_vgg = self.loss_dic['t_vgg'].get_loss(
                self.output_i, self.target_t)

            self.loss_G += self.loss_icnn_pixel+self.loss_icnn_vgg*self.opt.lambda_vgg
            self.loss_G += self.loss_icnn_remake*2
        else:
    ...
    def backward_R(self):
        ...
        if self.aligned:
            self.loss_icnn_pixel_r = self.loss_dic['r_pixel'].get_loss(
            ...
            self.loss_R += self.loss_icnn_vgg*self.opt.lambda_vgg+self.loss_icnn_pixel_r+self.loss_icnn_remake*2
        else:
            self.loss_CX = self.loss_dic['t_cx'].get_loss(self.output_i, self.target_t)
            
            self.loss_R += self.loss_CX
    ...
    def forward(self):
        ...
        output_i,input_r = self.net_i(input_i)
        output_r = self.net_r(input_r)
    ...
    def get_current_errors(self):
        ...
        if self.loss_icnn_pixel_r is not None:
            ret_errors['r_pixel'] = self.loss_icnn_pixel_r.item()
        if self.loss_icnn_remake is not None:
            ret_errors['remake'] = self.loss_icnn_remake.item()
    ...
    def load(model, resume_epoch=None):
        ...
        if icnn_path is None:
            ...
            model.net_r.load_state_dict(state_dict['icnn_r'])
            if model.isTrain:
                model.optimizer_G.load_state_dict(state_dict['opt_g'])
        else:
            state_dict = torch.load(icnn_path,map_location=torch.device('cpu'))
            model.net_i.load_state_dict(state_dict['icnn'])
            model.net_r.load_state_dict(state_dict['icnn_r'])
            model.epoch = state_dict['epoch']
            model.iterations = state_dict['iterations']
