...
class SubNet(torch.nn.Module):
    def __init__(self, in_channels, out_channels, n_feats, n_res_r, norm=nn.BatchNorm2d, 
    se_reduction=None, res_scale=1, bottom_kernel_size=3, pyramid=False):
        ...
        # Residual layers
        dilation_config = [1] * n_res_r

        self.res_r = nn.Sequential(*[ResidualBlock(
            n_feats, dilation=dilation_config[i], norm=norm, act=act, 
            se_reduction=se_reduction, res_scale=res_scale) for i in range(n_res_r)])
...
class LRNet(torch.nn.Module):
    def __init__(self, in_channels, out_channels, n_feats, n_resblocks_share, n_res_t, norm=nn.BatchNorm2d, 
    se_reduction=None, res_scale=1, bottom_kernel_size=3, pyramid=False):
        ...
        # Residual layers
        dilation_config = [1] * n_res_t

        self.res_module_share = nn.Sequential(*[ResidualBlock(
            n_feats, dilation=dilation_config[i], norm=norm, act=act, 
            se_reduction=se_reduction, res_scale=res_scale) for i in range(n_resblocks_share)])

        self.res_t=nn.Sequential(*[ResidualBlock(
            n_feats, dilation=dilation_config[i], norm=norm, act=act, 
            se_reduction=se_reduction, res_scale=res_scale) for i in range(n_res_t)])
    ...
    def forward(self,x):
        x_mid = self.forward1(x)
        x = self.forward2(x_mid)
