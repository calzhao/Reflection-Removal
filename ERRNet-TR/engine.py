...
from models.errnet_model import tensor2im
...
class Engine(object):
    ...
    def train(self, train_loader, **kwargs):
        ...
        for i, data in enumerate(train_loader):
            ...
            input_i,target_t,target_r=data['input'], data['target_t'], data['target_r']
            # savedir="./debug"
            # input_ = tensor2im(input_i)
            # name = os.path.splitext(os.path.basename(data['fn'][0]))[0]
            target_ = tensor2im(target_t)
            targetr_ = tensor2im(target_r)
            if(np.sum((target_-targetr_)**2)<1):
                self.iterations += 1
                # print(np.sum((target_-targetr_)**2))
                # print(name+"  ERROR")
                continue
            # Image.fromarray(target_.astype(np.uint8)).save(join(savedir,name+'t_label.png'))
            # Image.fromarray(input_.astype(np.uint8)).save(join(savedir,name+'m_input.png'))
            # Image.fromarray(targetr_.astype(np.uint8)).save(join(savedir,name+'r_label.png'))
